...
namespace CimmpleAPI.Services.Pdf
{
    public class DocumentPdfService
    {
        ...
        public DocumentPdfService(PdfService pdfService, CimmpleDbContext context, IWebHostEnvironment environment)
        {
            _pdfService = pdfService;
            _context = context;
            _environment = environment;
        }
        public async Task<DocumentPdfResult> BuildQuotationAsync(int quotationId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (quotation == null)
                    return new DocumentPdfResult { Error = "Quotation not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"Quotation_{quotationNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = quotationNumber,
                    DefaultToEmail = ResolveCustomerEmail(customer),
                    PartyName = quotation.CustomerName
                };
            }
            catch (Exception ex)
            {
                return new DocumentPdfResult { Error = ex.Message };
            }
        }
        public async Task<DocumentPdfResult> BuildOrderAsync(int orderId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (order == null)
                    return new DocumentPdfResult { Error = "Order not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"Order_{orderNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
        ...
        public async Task<DocumentPdfResult> BuildInvoiceAsync(int invoiceId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (invoice == null)
                    return new DocumentPdfResult { Error = "Invoice not found" };
                ...
                if (order == null)
                    return new DocumentPdfResult { Error = "Customer order not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"Invoice_{invoiceNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
        ...
        public async Task<DocumentPdfResult> BuildVendorInvoiceAsync(int invoiceId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (invoice == null)
                    return new DocumentPdfResult { Error = "Vendor invoice not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"VendorInvoice_{invoiceNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = invoiceNumber,
                    DefaultToEmail = ResolveVendorEmail(vendor),
        ...
        public async Task<DocumentPdfResult> BuildVendorOrderAsync(int orderId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (order == null)
                    return new DocumentPdfResult { Error = "Vendor order not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"VendorOrder_{orderNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = orderNumber,
                    DefaultToEmail = ResolveVendorEmail(vendor),
        ...
        public async Task<DocumentPdfResult> BuildVendorQuotationAsync(int quotationId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (quotation == null)
                    return new DocumentPdfResult { Error = "Vendor quotation not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"VendorQuotation_{quotationNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = quotationNumber,
                    DefaultToEmail = ResolveVendorEmail(vendor),
                    PartyName = quotation.VendorName
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<DocumentPdfResult> BuildShipmentAsync(int shipmentId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (shipment == null)
                    return new DocumentPdfResult { Error = "Shipment not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"Shipment_{shipmentNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = shipmentNumber,
                    DefaultToEmail = ResolveCustomerEmail(customer),
                    PartyName = order?.CustomerName
                };
            }
        ...
        public async Task<DocumentPdfResult> BuildJobOrderAsync(int jobOrderId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (jobOrder == null)
                    return new DocumentPdfResult { Error = "Job order not found" };
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"JobOrder_{jobOrderNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = jobOrderNumber,
                    DefaultToEmail = ResolveCustomerEmail(customer),
                    PartyName = jobOrder.CustomerName
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<DocumentPdfResult> BuildNcrAsync(int ncrId, int tenantId, int? locationId = null)
        {
            try
            {
                ...
                if (ncr == null)
                    return new DocumentPdfResult { Error = "NCR not found" };
                ...
                string? defaultToEmail = null;
                if (ncr.VendorId.GetValueOrDefault() > 0)
                {
                    var vendor = await _context.VendorMaster.AsNoTracking()
                        .FirstOrDefaultAsync(v => v.vendor_id == ncr.VendorId!.Value && v.Tenantid == tenantId);
                    defaultToEmail = ResolveVendorEmail(vendor);
                }
                else if (ncr.CustomerId.GetValueOrDefault() > 0)
                {
                    var customer = await _context.CustomerMaster.AsNoTracking()
                        .FirstOrDefaultAsync(c => c.customer_id == ncr.CustomerId!.Value && c.Tenantid == tenantId);
                    defaultToEmail = ResolveCustomerEmail(customer);
                }
                ...
                {
                    Bytes = pdfBytes,
                    FileName = $"NCR_{ncrNumber}_{DateTime.Now:yyyy-MM-dd}.pdf",
                    DocumentLabel = ncrNumber,
                    DefaultToEmail = defaultToEmail,
                    PartyName = ncr.CustomerName ?? ncr.VendorName
                };
            }
            catch (Exception ex)
            {
        ...
        private static string? ResolveCustomerEmail(CustomerMaster? customer)
        {
            if (customer == null) return null;
            var email = customer.ContactEmail ?? customer.email;
            return string.IsNullOrWhiteSpace(email) ? null : email.Trim();
        }

        private static string? ResolveVendorEmail(VendorMaster? vendor)
        {
            if (vendor == null) return null;
            var email = vendor.ContactEmail ?? vendor.email;
            return string.IsNullOrWhiteSpace(email) ? null : email.Trim();
        }
...
END");
        ...
        private async Task<NcrPdfRow?> LoadNcrForPdfAsync(int ncrId, int tenantId)
        {
            ...
            return new NcrPdfRow
            {
                ...
                CustomerId = ReadInt(reader, "CustomerId") > 0 ? ReadInt(reader, "CustomerId") : null,
                CustomerName = ReadString(reader, "CustomerName"),
                VendorId = ReadInt(reader, "VendorId") > 0 ? ReadInt(reader, "VendorId") : null,
