...
interface VendorInvoiceModalProps {
  ...
  onClose: () => void;
  orderId: number;
  invoiceableItems: InvoiceableItemForVendor[];
  selectedItems?: InvoiceableItemForVendor[];
  onInvoiceCreated: () => void;
}

const VendorInvoiceModal: React.FC<VendorInvoiceModalProps> = ({
  isOpen,
  onClose,
  orderId,
  invoiceableItems,
  ...
  const calculateLineTotal = (item: InvoiceableItemForVendor): number => {
  ...
  const invoiceTotal = Math.round((subtotal + taxAmount + freightCharge) * 100) / 100;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const request: CreateVendorInvoiceRequest = {
        orderId,
        lineItems,
      ...
      console.log('Creating vendor invoice with request:', request);

      const result = await VendorInvoiceService.CreateVendorInvoice(request);
      console.log('Vendor invoice creation result:', result);

      if (result) {
        console.log('Vendor invoice created successfully, closing modal');
        onClose();
        setTimeout(() => {
          console.log('Showing success toast and calling onInvoiceCreated');
          toast.success(`Vendor Invoice #${result.invoiceNumber} created successfully!`);
          onInvoiceCreated();
        }, 100);
      } else {
        console.log('No result from vendor invoice creation');
        toast.error('Failed to create vendor invoice - no response from server');
      }
    } catch (error: any) {
      console.error('Vendor invoice creation failed:', error);
      const errorMessage = error.response?.data?.error || error.message || 'Unknown error occurred';
      toast.error(`Failed to create vendor invoice: ${errorMessage}`);
    } finally {
        ...
        <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                          Available: {item.availableQty} units • Received: {item.receivedQty} • Invoiced: {item.invoicedQty}
                        </div>
                      </div>
          ...
          <div className="modal-footer" style={{
            ...
            <button
              ...
              style={{
                padding: '0.5rem 1rem',
                backgroundColor: loading || !invoiceNo || invoiceNo.trim() === '' ? '#9ca3af' : '#8b5cf6',
                color: 'white',
                border: 'none',
                borderRadius: '0.375rem',
                cursor: loading || !invoiceNo || invoiceNo.trim() === '' ? 'not-allowed' : 'pointer',
                fontSize: '0.875rem',
                fontWeight: '500'
              }}
            >
