...
import { PunchDirection, PunchInOutService } from "../../Common/Services/PunchInOutService";
...
const PUNCH_ADMIN_UNLOCK_KEY = "punchAdminUnlocked";
const CAMERA_SCAN_INTERVAL = 2000; // Scan every 2 seconds
...
const PunchWorkflowPanel: React.FC<{
  ...
  onSubmit,
    ...
    const canSubmit = !!selectedEmployee && !captureLoading && !passwordLoading && (!isNewEmployee || passwordEntry.trim().length > 0);
    const submitLabel = isNewEmployee
      ? "Verify & Register"
      : passwordEntry.trim()
        ? "Verify Password"
        : "Capture & Punch";
              ...
              <div className="punch-password-block">
                ...
                <div className="punch-password-row">
                  <input
                    ...
                    placeholder={isNewEmployee ? "Required for new users" : "Optional for existing users"}
                    value={passwordEntry}
...
export const PunchInOut: React.FC = () => {
  ...
  const getPunchToken = (): string => {
    ...
    try {
      ...
      const session = JSON.parse(rawCookie);
      if (Number(session?.rolId) !== 1 || !session?.token || isPunchSessionExpired(session)) {
        clearPunchSession();
        return "";
      }
      localStorage.setItem(PUNCH_TOKEN_KEY, session.token);
  ...
  const isAdminPunch = Number(punchStorage?.rolId) === 1;
  ...
  const handlePunchLogout = () => {
    ...
    Utils.setCookie(PUNCH_SESSION_COOKIE, "", 0);
    window.location.href = "/punch-login";
  };
  ...
  const handleAdminUnlock = React.useCallback(async () => {
    ...
    try {
      const user = await User.login(punchAdminUserName, adminUnlockPassword, "");
      if (user && Number(user?.rolId) === 1) {
  ...
  if (!isPunchAuthorized) {
    return <Redirect to="/punch-login" />;
  }
      ...
      {adminUnlockVisible && isAdminPunch && !adminModeUnlocked && (
        <div className="admin-unlock-overlay" role="presentation" onClick={closeAdminUnlockModal}>
          <div className="admin-unlock-modal" role="dialog" aria-modal="true" aria-labelledby="admin-unlock-title" onClick={(event) => event.stopPropagation()}>
            ...
            <span className="admin-unlock-badge">ADMIN SECURITY</span>
            <h2 id="admin-unlock-title">Unlock Admin Mode</h2>
            <p>Enter your password to manage attendance &amp; view company status.</p>
            ...
            >
              {adminUnlockLoading ? "Verifying..." : "Unlock Admin Mode"}
            </button>
            <span className="admin-unlock-hint">Only company admins can unlock attendance board views.</span>
          </div>
        </div>
      )}
