...
/** Append report period so related screens match the report date filter. */
function withPeriod(
  path: string,
  startDate?: string | null,
  endDate?: string | null
): string {
  if (!startDate && !endDate) return path;
  return withQuery(path, {
    startDate: startDate || undefined,
    endDate: endDate || undefined,
    dateRange: startDate || endDate ? "Custom" : undefined,
  });
}
...
export function buildDrillLink(opts: DrillLinkOptions): string {
  ...
  // Sales-trends month bucket (yyyy-MM) → month start/end when report period not narrower
  let periodStart = opts.startDate;
  let periodEnd = opts.endDate;
  if (
    entityType === "invoice-period" &&
    opts.entityKey &&
    /^\d{4}-\d{2}$/.test(opts.entityKey.trim())
  ) {
    const [y, m] = opts.entityKey.trim().split("-").map((x) => parseInt(x, 10));
    if (y > 0 && m >= 1 && m <= 12) {
      const lastDay = new Date(y, m, 0).getDate();
      periodStart = `${opts.entityKey.trim()}-01`;
      periodEnd = `${opts.entityKey.trim()}-${String(lastDay).padStart(2, "0")}`;
    }
  }
  ...
  if (entityType === "location") {
    return withLocationId(
      withPeriod(rawPath, periodStart, periodEnd),
      opts.locationId
    );
  }
  ...
  if (isAggregate) {
    const search =
      ...
      null;
    if (entityType === "invoice-period") {
      return withLocationId(
        withPeriod(rawPath, periodStart, periodEnd),
        opts.locationId
      );
    }
    if (search && entityType !== "process") {
      ...
      if (entityType === "customer" || entityType === "vendor" || entityType === "vendor-part") {
        ...
        return withLocationId(
          withPeriod(path, periodStart, periodEnd),
          opts.locationId
        );
      }
    }
